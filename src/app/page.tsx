...
import {
  ProfileUploadDialog,
  ProfileUploadDialogRef,
} from "@/components/ProfileUploadDialog";
...
function cleanProfiles(profiles: any[], statuses: any[], tags: any[]) {
  const statusMap = Object.fromEntries(
    statuses.map((s: any) => [s.id, s.status]),
  );
...
export default function HomePage() {
  ...
  useEffect(() => {
    ...
    getProfilesWithMeta(selectedFolderId).then(
      ({ profiles, statuses, tags }) => {
  ...
  const handleDelete = async () => {
    ...
    const { profiles, statuses, tags } =
      await getProfilesWithMeta(selectedFolderId);
    setProfiles(cleanProfiles(profiles, statuses, tags));
  };
