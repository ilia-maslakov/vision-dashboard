...
export async function GET(req: NextRequest) {
  ...
  const { data: { items: allProfiles = [] } = {} } = await fetchJson(
    `${BASE}/folders/${folderId}/profiles?ps=1000`,
  );

  const profiles = filterIds
    ? allProfiles.filter((p) => filterIds.includes(p.id))
    : allProfiles;
