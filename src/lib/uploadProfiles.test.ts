...
describe('uploadProfiles', () => {
  ...
  it('calls createProfile and uploadCookies correctly and returns { status: "ok" }', async () => {
    ...
    // Мокаем createProfile и uploadCookies
    const createProfileMock = vi.spyOn(createProfileModule, 'createProfile').mockResolvedValue('profile-id')
    const uploadCookiesMock = vi.spyOn(uploadCookiesModule, 'uploadCookies').mockResolvedValue(undefined)

    const result = await uploadProfilesModule.uploadProfiles(folderId, profiles)
    ...
    expect(uploadCookiesMock).toHaveBeenCalledWith(folderId, 'profile-id', profiles[0].cookie)
  ...
  it('catches errors and logs them without throwing', async () => {
    ...
    const createProfileMock = vi.spyOn(createProfileModule, 'createProfile').mockRejectedValue(new Error('createProfile failed'))
    const uploadCookiesMock = vi.spyOn(uploadCookiesModule, 'uploadCookies').mockResolvedValue(undefined)
