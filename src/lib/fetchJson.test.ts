...
describe('fetchJson', () => {
  ...
  it('fetches JSON successfully with default headers', async () => {
    ...
    expect(global.fetch).toHaveBeenCalledWith('https://example.com/api/data', expect.objectContaining({
  ...
  it('merges additional headers correctly', async () => {
    ...
    const result = await fetchJson('https://example.com/api/data', { headers: extraHeaders })
