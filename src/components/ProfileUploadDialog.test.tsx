...
describe("ProfileUploadDialog", () => {
  ...
  it("should open modal with descriptions after file load and allow confirm selection", async () => {
    ...
    const file = new File([fileContent], "profiles.json", {
    ...
    await waitFor(() => {
      expect(
        screen.getByText(/Выберите профили для загрузки/),
      ).toBeInTheDocument();
    });
  ...
  it("should close modal on cancel", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.queryByText(/Выберите профили для загрузки/),
