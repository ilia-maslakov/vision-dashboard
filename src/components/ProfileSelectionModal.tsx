...
export function ProfileSelectionModal(
    ...
    const [selected, setSelected] = useState<string[]>(profiles.map(p => p.name))
    ...
    const modalContent = (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div
                className="bg-white dark:bg-zinc-900 text-black dark:text-white p-6 rounded-md shadow-md w-[700px] max-h-[80vh] flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto">
                    <table className="w-full text-sm">
                        ...
                        <tbody>
                        {profiles.map(p => (
                            <tr key={p.name} className="border-t border-zinc-800 hover:bg-zinc-800/40">
